...
# Hardcoded credentials for your 3-person team (change usernames/passwords as desired)
...
# --- Gemini API Setup ---
# Automatically pulls from environment variable (set GEMINI_API_KEY in your hosting provider)
...
def parse_email_with_ai(email_text):
  ...
  try:
    ...
    # Basic cleanup to parse dict from response text safely in production
    # For robust production, use structured outputs, but this serves well in prototype:
    import ast
...
with st.expander("📥 Drag & Drop New Bid Invitation (.msg or .eml)", expanded=False):
  uploaded_file = st.file_uploader(
      "Upload Outlook Email File", type=["msg", "eml"]
  )
  if uploaded_file is not None:
    ...
    if uploaded_file.name.endswith(".msg"):
      ...
      msg = email.message_from_bytes(
          uploaded_file.read(), policy=policy.default
      )
      raw_text = (
    ...
    if st.button("Process Email with AI"):
      with st.spinner("Extracting project specifications..."):
        parsed_data = parse_email_with_ai(raw_text)
